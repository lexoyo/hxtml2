...
hxtml2.HTMLPageData = function(cssParser) {
	...
	this._ids = new Hash();
	this.containerDOMElement = new cocktailCore.domElement.js.ContainerDOMElement();
}
hxtml2.HTMLPageData.__name__ = ["hxtml2","HTMLPageData"];
hxtml2.HTMLPageData.prototype.containerDOMElement = null;
...
hxtml2.HTMLPageData.prototype.createElement = function(elementType,attributes,parent,semantic) {
	...
	if(elementType != hxtml2.ElementTypeValue.body && parent == null) parent = this.containerDOMElement;
	var element = null;
	switch( (elementType)[1] ) {
	case 0:
		element = new cocktailCore.domElement.js.ImageDOMElement();
		if(attributes.exists("src")) element.load(attributes.get("src"));
		break;
	case 1:
		element = this.containerDOMElement;
		break;
	default:
...
hxtml2.HTMLParser.prototype._doParse = function(htmlDOM,htmlPageData,parent) {
	...
	default:
		...
		var $it0 = htmlDOM.attributes();
		while( $it0.hasNext() ) {
			var attr = $it0.next();
			attributesHash.set(attr,htmlDOM.get(attr));
		}
		element = htmlPageData.createElement(elementType,attributesHash,parent,htmlDOM.getNodeName());
	}
	var hasText = false;
	var $it1 = htmlDOM.iterator();
	while( $it1.hasNext() ) {
		var child = $it1.next();
