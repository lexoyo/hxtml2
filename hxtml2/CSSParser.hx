...
typedef EnumStringConnection =
{
	var enumValue:Dynamic; 
	var stringValue:String;
}

class EnumStringConnectionManager
{
	public static inline var enumStringConnectionArray:Array<Dynamic> = 
	[
		{
			enumValue:_in, 
			stringValue:"in"
		...
		{
			enumValue:staticStyle, 
			stringValue:"static"
		}
	];
	public static function getEnumFromStringValue(enumType:Dynamic, stringValue:String, arguments:Array<Dynamic>=null):Dynamic
	{
		trace("getEnumFromStringValue("+Type.typeof(enumType)+", "+stringValue+", "+arguments+")");
		
		// find the enum corresponding to the value
		for (idx in enumStringConnectionArray)
		{
			if (idx.stringValue == stringValue)
			{
				if (arguments == null)
					return idx.enumValue;
					
				trace(idx.stringValue + " found as "+idx.enumValue);
		...
		// if it is not in the list, try to convert it automatically
		try
		{
...
class CSSParser
{
	...
	/**
	 * Apply style to an element from a CSS rule, i.e. with selectors
	 ...
	 */
/*	public function setStyles(style:Style, cssRules:Array<CSSRule>):Void
	{
		throw "to be implmented";
	}
*/	

	function applyStyle( styleName : String, v : Value, s : Style ) : Bool 
	{
		...
		switch (v)
		{
			case VUnit(value, unit):
				...
				{
						case "width":
							//thers is a haxe bug here ? Reflect.setField(s, styleName, DimensionStyleValue.percent(Math.round(value)));
							s.width = getStyleValue(DimensionStyleValue, value, unit);
						case "height":
							s.height = getStyleValue(DimensionStyleValue, value, unit);
						case "top":
							s.top = getStyleValue(PositionOffsetStyleValue, value, unit);
						case "left":
							s.left = getStyleValue(PositionOffsetStyleValue, value, unit);
						case "right":
							s.right = getStyleValue(PositionOffsetStyleValue, value, unit);
						case "bottom":
							s.bottom = getStyleValue(PositionOffsetStyleValue, value, unit);
						case "font-size":
							s.fontSize = getStyleValue(FontSizeStyleValue, value, unit);
						case "margin-left":
							s.marginLeft = getStyleValue(MarginStyleValue, value, unit);
				}
			case VIdent(value):
				switch(styleName) 
				{
					case "display":
						s.display = EnumStringConnectionManager.getEnumFromStringValue(DisplayStyleValue, value);
					case "position":
						s.position = EnumStringConnectionManager.getEnumFromStringValue(PositionStyleValue, value);
				}
/*			case VGroup(array):
				switch(styleName) 
				{
					case "margin":
						// to do
						s.marginLeft = EnumStringConnectionManager.getEnumFromStringValue(DisplayStyleValue, array[0]);
				}
*/				
			default:
				trace("unknown style \""+styleName+"\" of type "+v);
				return false;
		}
		return true;
	}
	function getStyleValue(enumType:Dynamic, value:Dynamic, unit : Null<String> = null):Dynamic
	{
		trace("getStyleValue "+enumType+", "+value+", "+unit);
		return switch (unit)
		{
			// percent case
			case "%":
				enumType.percent(Math.round(value)); 
			default:
				enumType.length(EnumStringConnectionManager.getEnumFromStringValue(LengthValue, unit, [value]));
		}
	}

/*	function getLengthValue(value:Float, unit : String):LengthValue
	{
		var lengthValueType:LengthValue;
		try
		{
			lengthValueType = Type.createEnum(LengthValue, unit, [value]);
		}catch(msg : String)
		{
